...
export async function generatePrompt(code: string): Promise<string> {
  try {
    ...
    const processedJobData = {
      ...
      abilities: jobData.abilities.map(a => a.abilities?.replace(/ Related occupations$/, '')),
      activities: jobData.activities.map(a => a.activity?.replace(/ Related occupations$/, '')),
      knowledge: jobData.knowledge.map(k => k.knowledge?.replace(/ Related occupations$/, '')),
      skills: jobData.skills.map(s => s.skill?.replace(/ Related occupations$/, '')),
      tasks: jobData.tasks.map(t => t.task?.replace(/ Related occupations$/, '')),
      ...
      tools: jobData.tools.map(t => t.tools?.replace(/ Related occupations$/, '')),
      values: jobData.values.map(v => v.value?.replace(/ Related occupations$/, '')),
    ...
    if (processedJobData.featured) {
      promptLines.push(`**Featured Occupation:** This occupation is gaining popularity.`);
    }
    ...
    if (processedJobData.related_experience) {
      promptLines.push(`- **Related Experience:** ${processedJobData.related_experience}`);
    }
    ...
    if (processedJobData.median_wage !== undefined && processedJobData.median_wage !== null) {
      promptLines.push(`- **Median Wage:** Approximately **${processedJobData.median_wage} Kazakhstani Tenge**.`);
    }
    if (processedJobData.employment_growth) {
      promptLines.push(`- **Employment Growth:** ${processedJobData.employment_growth}`);
    }
    if (processedJobData.job_openings !== undefined && processedJobData.job_openings !== null) {
      promptLines.push(`- **Job Openings:** About **${processedJobData.job_openings}** positions available.`);
    }
    if (processedJobData.emloyees !== undefined && processedJobData.emloyees !== null) {
      promptLines.push(`- **Number of Employees:** Around **${processedJobData.emloyees}** individuals are employed in this occupation.`);
    }

    promptLines.push(`\n---\n`);

    // Abilities
    if (Array.isArray(processedJobData.abilities) && processedJobData.abilities.length > 0) {
    ...
    // Activities
    if (Array.isArray(processedJobData.activities) && processedJobData.activities.length > 0) {
    ...
    // Knowledge Areas
    if (Array.isArray(processedJobData.knowledge) && processedJobData.knowledge.length > 0) {
    ...
    // Skills
    if (Array.isArray(processedJobData.skills) && processedJobData.skills.length > 0) {
      ...
      if (processedJobData.skills.length > 5) {
        promptLines.push(`*Additional skills include ${processedJobData.skills.length - 5} more skills.*`);
      }
      promptLines.push(`\n---\n`);
    }

    // Tasks
    if (Array.isArray(processedJobData.tasks) && processedJobData.tasks.length > 0) {
    ...
    if (Array.isArray(processedJobData.technology) && processedJobData.technology.length > 0) {
      promptLines.push(`- **Software:** ${processedJobData.technology.join(", ")}.`);
    }
    if (Array.isArray(processedJobData.tools) && processedJobData.tools.length > 0) {
    ...
    // Work Values
    if (Array.isArray(processedJobData.values) && processedJobData.values.length > 0) {
    ...
    if (Array.isArray(processedJobData.specs) && processedJobData.specs.length > 0) {
      promptLines.push(`**Educational Pathways in Kazakhstan:**\n`);
      promptLines.push(`To pursue this career, consider the following university specializations:\n`);
      processedJobData.specs.forEach((spec) => {
        ...
        if (spec.group_name) promptLines.push(`- **Group Name:** ${spec.group_name}`);
        if (spec.subjects) promptLines.push(`- **Required Subjects for ЕНТ:** ${spec.subjects}`);
        if (spec.count) promptLines.push(`- **Scholarships Last Year:** ${spec.count}`);
        ...
        if (Array.isArray(spec.universities) && spec.universities.length > 0) {
          ...
          spec.universities.forEach((univer, index) => {
            if (univer.name_ru) promptLines.push(`\n${index + 1}. **${univer.name_ru} ${univer.abbreviation ? `(${univer.abbreviation})` : ''}**`);
            if (univer.code) promptLines.push(`   - **Code:** ${univer.code}`);
            if (univer.address) promptLines.push(`   - **Location:** ${univer.address}`);
            if (univer.region_name) promptLines.push(`   - **Region:** ${univer.region_name}`);
            if (univer.university_type) promptLines.push(`   - **University Type:** ${univer.university_type}`);
            if (univer.website) promptLines.push(`   - **Website:** ${univer.website}`);
            if (univer.social) promptLines.push(`   - **Social Media:** ${univer.social}`);
            if (univer.phone1 || univer.phone2) {
              promptLines.push(`   - **Contact:** ${univer.phone1}${univer.phone2 ? ", " + univer.phone2 : ""}`);
            }
            promptLines.push(`   - **English Program Availability:** ${univer.is_english_available === "yes" ? "Yes" : "No"}`);
            if (univer.min_grant) promptLines.push(`   - **Scholarships:** ${univer.min_grant}`);
            if (univer.min_point_gen_2023) promptLines.push(`   - **Minimum ЕНТ Score for General Scholarship:** ${univer.min_point_gen_2023}`);
            if (univer.count_grant_gen_2023) promptLines.push(`   - **General Scholarships Available:** ${univer.count_grant_gen_2023}`);
            if (univer.min_point_aul_2023) promptLines.push(`   - **Minimum ЕНТ Score for Rural Quota Scholarship:** ${univer.min_point_aul_2023}`);
            if (univer.count_grant_aul_2023) promptLines.push(`   - **Rural Quota Scholarships Available:** ${univer.count_grant_aul_2023}`);
            if (univer.teacher_quota) promptLines.push(`   - **Teacher Quota:** ${univer.teacher_quota}`);
            if (univer.serpin_quota) promptLines.push(`   - **Serpin Quota:** ${univer.serpin_quota}`);
            if (univer.military_factory) promptLines.push(`   - **Military Education Included:** ${univer.military_factory}`);
            if (univer.dormitory) promptLines.push(`   - **Dormitory Available:** ${univer.dormitory}`);
          });
        }
        promptLines.push(`\n`);
      });
    ...
    promptLines.push(`- **Median Wage:** The median wage is provided in Kazakhstani Tenge.`);
    if (processedJobData.featured) {
      promptLines.push(`- **Featured Occupation:** This occupation is gaining popularity.`);
    }
    promptLines.push(`- **Educational Pathways:** Understanding the necessary subjects for the ЕНТ exam and the universities offering relevant programs can guide prospective students.`);
    promptLines.push(`- **University Quotas and Facilities:** Information about teacher quotas, Serpin quotas, military education, and dormitory availability is provided.`);
    promptLines.push(`\n---\n`);
